...
# הגדרות דף
st.set_page_config(
    page_title=" דאבל בעיצוב אישי - יצירת משחק קלפים בעיצוב שלך",
    page_icon="🎴",
    layout="centered",
    ...
    menu_items={
        ...
        'Report a bug': None,
        'About': """
            ### דאבל בעיצוב אישי - יצירת משחק קלפים בעיצוב שלך
...
if uploaded_files and len(uploaded_files) >= total_needed:
    ...
    if st.button("✨ צור משחק וקובץ להורדה"):
        ...
        total_cards = len(deck_indices)
        
        # לולאת יצירת הקלפים
        for idx, card_indices in enumerate(deck_indices):
            # --- שלב 2: עדכון מד ההתקדמות בתוך הלולאה ---
            percent_complete = int(((idx + 1) / total_cards) * 100)
            progress_bar.progress(percent_complete)
            status_text.text(f"מעבד קלף {idx + 1} מתוך {total_cards} ({percent_complete}%)")
            ...
            # פרמטרים דינמיים לפי כמות הסמלים
            if num_on_card <= 3:
        ...
        # --- שלב 3: ניקוי המד בסיום ---
        progress_bar.empty()
        status_text.success(f"✅ כל {total_cards} הקלפים מוכנים!")
